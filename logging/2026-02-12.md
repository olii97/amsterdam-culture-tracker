...
### 6.1 Date parsing — why only Concertgebouw and De Balie appeared in the DB

Events are only written to Supabase when `parse_event_date()` returns a date. Other venues use formats the old parser didn’t handle, so their events were skipped (“Skipped 422 event(s) with unparseable dates”).

| Venue | Example date string | Before | After |
|-------|---------------------|--------|--------|
| Concertgebouw | `do 12 feb 2026, 20:15` | ✅ Parsed | ✅ Parsed |
| De Balie | `vr 13 februari 2026, 19:30` | ✅ Parsed | ✅ Parsed |
| Paradiso / Rode Hoed | `do 12 feb. 2026, 00:00` (dot after month) | ❌ Failed | ✅ Parsed |
| Pakhuis / De Kleine Komedie | `ma 16 feb, 19.30` (no year) | ❌ Failed | ✅ Parsed (year inferred) |
| Pakhuis | `Vandaag, 20.00` | ❌ Failed | ✅ Parsed (today) |

**Changes in `supabase_writer.py` (`parse_event_date()`):**

1. **Optional dot after month** — e.g. `feb. 2026` (Paradiso, Rode Hoed).
2. **No year → infer current year** — e.g. `ma 16 feb, 19.30`; if that date is already in the past, use next year.
3. **“Vandaag”** — return today’s date.
4. **Fallback** — keep existing pattern for `DD month YYYY` (e.g. De Balie’s “13 februari 2026”).

After re-running the scraper, far fewer events are skipped and Paradiso, Rode Hoed, De Kleine Komedie, and Pakhuis de Zwijger events appear in the database and app.
...
| Scraper  | `supabase_writer.py` (dedup, scraper_runs logging, date parsing for all venues) |
